...
def urls_for_dates(days='1') -> list:
...
async def get_exchange(currency_code='EUR', days='1'):
    exchange_result = []
    urls = urls_for_dates(days)
    ...
    if results:
        for result in results:
            ...
            exchange_result.append(f"{currency_code}: buy: {exchange['purchaseRateNB']}, sale: "
                                   f"{exchange['saleRateNB']}. Date: {result.get('date')}")
